def make_slider(prompt_electrons, unmatched_electrons):
    ...
    def calculate_efficiency(sig, bkg, feature1_cut, feature2_cut, feature3_cut):
        signal = sig[
            (sig.sieie < feature1_cut)
            & (sig.hoe < feature2_cut)
            & (sig.pfRelIso03_chg < feature3_cut)
        ]
        background = bkg[
            (bkg.sieie < feature1_cut)
            & (bkg.hoe < feature2_cut)
            & (bkg.pfRelIso03_chg < feature3_cut)
        ]
    ...
    def update_efficiency(feature1_cut, feature2_cut, feature3_cut):
        signal_efficiency, background_efficiency = calculate_efficiency(
            sig, bkg, feature1_cut, feature2_cut, feature3_cut
        )
    ...
    ele_hadronicOverEm_slider = widgets.FloatSlider(
        value=0.2,
        min=min(prompt_electrons.hoe),
        max=0.2,
        step=0.01,
        description="hadronicOverEm:",
    )
    ele_sieie_slider = widgets.FloatSlider(
        value=max(prompt_electrons.sieie),
        min=min(prompt_electrons.sieie),
        max=0.05,
        step=0.001,
        description="sieie:",
    )
    ele_pfRelIso03_chg_slider = widgets.FloatSlider(
        value=1,
        min=min(prompt_electrons.pfRelIso03_chg),
        max=1,
        step=0.01,
        description="pfRelIso03:",
    )
