...
def reco_electron_pairs(pT1, pT2, eta1, eta2, phi1, phi2,calib,laser_corrections_scenario=2):
...
def make_mass_plot(gen_m_ee,reco_m_ee,alpha_Calib,laser_scenario,pT1, pT2, eta1, eta2, phi1, phi2):
    from matplotlib.widgets import Slider,CheckButtons # Import Slider class
    ################################################################
    def plotz(ax,gen_m_ee,reco_m_ee):
        hist = ax.hist([gen_m_ee/1000,reco_m_ee/1000], histtype='step', linewidth=2,
                       bins=np.arange(60, 120, 0.5).tolist(),label=["generated electron pairs","reconstructed electron pairs"]) # Divide by 1000 to convert from MeV to GeV
    ...
    slider_ax = fig.add_axes([0.3, 0.05, 0.5, 0.03]) # Position and size of the slider axes
    slider_ax_2 = fig.add_axes([0.3, 0.1, 0.5, 0.03]) # Position and size of the slider axes
    calib_slider = Slider(slider_ax, 'alpha_Calib', 0.9, 1.2, valinit=alpha_Calib)
    scenario_slider = Slider(slider_ax_2, 'laser scenario', 0, 2, valinit=laser_scenario,valstep=1)
    ...
    def update(val):
        ...
        recopT1, recopT2, recoeta1, recoeta2, recophi1, recophi2=reco_electron_pairs(pT1, pT2, eta1, eta2, phi1, phi2,calib,scenario)
        # Recalculate reco_m_ee using the updated recopT1 and recopT2
        reco_m_ee = calculate_invariant_mass(recopT1, recopT2, recoeta1, recoeta2, recophi1, recophi2)
